...
TreeBSP *ObjectBSP::consBSP(const vector<FaceBSP *> &tabFace) {
    ...
    if (tabFace.empty()) {
        ...
        res->node(pivot); // ajouter à tree

        // POUR CHAQUE autre face
        for(int i=1;i<tabFace.size();i++) {
            FaceBSP *face = tabFace[i];
            face->separe(*pivot); // déterminer le positif du nega par rapport au pivot

            // AJOUTER face negative
            if(face->faceNegative() != NULL) {
                listeNegative.push_back(face->faceNegative());
            }

            // AJOUTER face positive
            if(face->facePositive() != NULL) {
                listePositive.push_back(face->facePositive());
            }
        }
...
void ObjectBSP::drawBSP(TreeBSP *tree,const Vector3 &eye) {
    ...
    if(tree != NULL) {
        // AFFICHER B
        FaceBSP *B = tree->node();
        ESign signeEyeDepuisB = B->sign(eye);

        // SI [eye EST DEVANT forme]
        if(signeEyeDepuisB == SIGN_PLUS){
            drawBSP(tree->left(), eye); // AFFICHER negatif (derriere)
            B->addDraw(); // tracer B
            drawBSP(tree->right(), eye); // AFFICHER positif (devant)

        } else { // SINON
            drawBSP(tree->right(), eye); // AFFICHER positif (derriere)
            B->addDraw(); // tracer B
            drawBSP(tree->left(), eye); // AFFICHER negatif (devant)
        }
    }
}
