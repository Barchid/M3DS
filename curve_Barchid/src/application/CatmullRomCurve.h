...
class CatmullRomCurve: public Curve {
    ...
    Curve cubic(double t,double *tCubic);




    p3d::Matrix4 tbn(double tValue);
    p3d::Matrix4 tbnManual(double t);
    void whichCurve(double t, int *cubicNumber, double *tCubic);
    p3d::Vector3 interpolateAcceleration(double t);
    Curve cubic(unsigned int i);
    void drawTBNManual();
    void drawTBNManual(double t);
    double interpolateAngle(double t);
private:
