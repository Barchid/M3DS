...
void CatmullRomCurve::draw() {
    ...
    // POUR CHAQUE [point saisi (sauf le dernier)]
    for(unsigned int i = 0; i < nbPoint() - 1; i++) {
        // CREER la courbe
        cubic.point(0, point(i)); // point de saisi du début
        cubic.point(1, intermediate(i, 0)); // point intermédiaire 1
        cubic.point(2, intermediate(i, 1)); // point intermédiaire 2
        cubic.point(3, point(i + 1)); // point de saisi de fin

        // DESSINER la courbe de bézier entre les points saisis
        cubic.drawBezier();
    }
}
...
void CatmullRomCurve::setup() {
    ...
    float k = 0.4; // définir empiriquement une constante pour Catmull...
    // POUR CHAQUE [point saisi]
    for(unsigned int i=1;i<nbPoint() - 1;i++) {
        // Calcul du segment Pi-1 Pi
        parallele = point(i + 1) - point(i - 1);

        // placement des points intermédiaires
        // On place les points intermédiaires de part et d'autre du point de saisi courant
        this->intermediate(i - 1, 1, point(i) - (0.4 * parallele) / 2);
        this->intermediate(i, 0, point(i) + (0.4 * parallele) / 2);
    }
}
